...
def canonical_answer_label(token: Any) -> Optional[str]:
    ...
    for label, aliases in _LABEL_ALIASES.items():
        if text in aliases:
            return label
    # UNSURE is multi-token on some vocabularies ('UN'+'S'+'URE'), and the three
    # labels start with distinct letters, so an unambiguous prefix is safe.
    if len(text) >= 2:
        hit = {
            label
            for label, aliases in _LABEL_ALIASES.items()
            for a in aliases
            if len(a) > len(text) and a.startswith(text)
        }
        if len(hit) == 1:
            return hit.pop()
    return None
